...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ __('Create Competency') }}</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-1">{{ __('Create Competency') }}</h3>
        </div>
        <div class="card-body">

            <form method="POST" action="{{ route('competency.store', $employee) }}" accept-charset="UTF-8"
                    ...
                    <div class="form-group col-md-12 {{ $errors->has('title') ? 'has-error' : '' }}">
                        ...
                        <div class="col-md-12">
                            <input type="text" name="title" id="title" class="form-control "
                                value="{{ Request::old('title') ?: '' }}" placeholder="name">
                            <span
    ...
    <script type="text/javascript">
        ...
        $("#add").click(function() {
            ...
            $("#dynamicTable").append(
                ...
                i +
                '][name]" id="name" class="form-control " value="{{ Request::old('name') ?: '' }}"placeholder="Competency Name"><span class="text-danger">{{ $errors->first('name') }}</span></div></div><div class="col-md-4"><label  for="remove">Action</label><div class="input-group"><button type="button" class="btn btn-danger remove-tr1">Remove</button></div></div></div></tr1>'
                );
        });
