...
<li class="breadcrumb-item active">{{(__('Create Weight'))}}</li>
@endsection
@section('content')
<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title mb-1">{{(__('Create Weight'))}}</h3>
    </div>
    <div class="card-body">

        <form method="POST" action="{{ route('weight.store', $employee) }}" accept-charset="UTF-8"
            ...
            <div class="row">
                <div class="form-group col-md-12 {{ $errors->has('title') ? 'has-error' : '' }}">
                    <label for="title" class="col-md-12 control-label">{{ __('Name') }} <span
                ...
                <div class="form-group col-md-12 {{ $errors->has('total_weight') ? 'has-error' : '' }}">
                    ...
                    <div class="col-md-12">
                        <input type="number" name="total_weight" id="total_weight" class="form-control "
                            value="{{Request::old('total_weight') ? : ''}}" placeholder="total_weight">
                        <span
