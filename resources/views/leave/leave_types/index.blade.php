        ...
        <table class="table table-striped ">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <form method="POST" action="{!! route('leave_types.leave_type.destroy', $leaveType->id) !!}"
                            accept-charset="UTF-8">
                            @method('DELETE')
                            {{ csrf_field() }}
                            <div class="btn-group btn-group-xs pull-right" role="group">
                                @permission('leave_types_show')
                                <a href="{{ route('leave_types.leave_type.show', $leaveType->id ) }}"
                                    class="btn btn-primary" title="Show Leave Type">
                                    <span class="fa fa-eye" aria-hidden="true"></span>
                                </a>
                                @endpermission
                                @permission('leave_types_edit')
                                <a href="{{ route('leave_types.leave_type.edit', $leaveType->id ) }}"
                                    class="btn btn-warning" title="Edit Leave Type">
                                    <span class="fa fa-edit text-white" aria-hidden="true"></span>
                                </a>
                                @endpermission
                                @permission('leave_types_delete')
                                <button type="submit" class="btn btn-danger" title="Delete Leave Type"
                                    onclick="return confirm(&quot;Click Ok to delete Leave Type.&quot;)">
