...
<li class="breadcrumb-item"><a
        href="{{ route('job_positions.job_position.index') }}">{{(__('setting.JobPosition'))}}</a></li>
...
<div class="card card-primary">
    <div class="card-header clearfix">
        <h4 class="card-title">{{(__('setting.view_job'))}}</h4>
        <div class="card-tools">
            <form method="POST" action="{!! route('job_positions.job_position.destroy', $jobPosition->id) !!}"
                accept-charset="UTF-8">
                @method('DELETE')
                {{ csrf_field() }}
                <div class="btn-group btn-group-sm" role="group">
                    @permission('jobsPosition-Edit')
                    <a href="{{ route('job_positions.job_position.edit', $jobPosition->id) }}" class="btn btn-warning"
