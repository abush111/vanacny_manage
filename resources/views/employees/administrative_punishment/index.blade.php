...
<div class="card card-primary">
    ...
    <div class="card-body">
        @permission('administrativePunishments-List')
        @if (count($employeeAdministrativePunishments) == 0)
        <h4 class="text-center">{{(('employee.No Administrative Punishments Available'))}}.</h4>
        @else
        <table class="table table-striped ">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <form method="POST"
                            action="{!! route('employee_administrative_punishments.employee_administrative_punishment.destroy', ['employee' => $employeeAdministrativePunishment->employees->id, 'employeeAdministrativePunishment' => $employeeAdministrativePunishment->id]) !!}"
                            accept-charset="UTF-8">
                            @method('DELETE')
                            {{ csrf_field() }}
                            <div class="btn-group btn-group-xs pull-right" role="group">
                                @permission('administrativePunishments-Edit')
                                ...
                                @endpermission
                                @permission('administrativePunishments-Delete')
                                <button type="submit" class="btn btn-danger" title="Delete Administrative Punishment"
...
@permission('administrativePunishments-AddNew')
...
@endpermission
@if (count($employeeAdministrativePunishments) > 0)
@permission('administrativePunishments-Print')
<a href="{{ route('employee_administrative_punishments.employee_administrative_punishment.print', $employee) }}"
    class="btn btn-primary" title="Print Employee Administrative Punishment" target="_blank">
