        ...
        <div class="col-md-12">
            <select class="form-control" id="title" name="title">
                <option value="" style="display: none;"
                    {{ old('title', optional($employee)->title ?: '') == '' ? 'selected' : '' }}
                    disabled selected>
...
<div class="row">
    <div class="form-group col-md-4 {{ $errors->has('sex') ? 'has-error' : '' }}">
        <label for="sex" class="col-md-12 control-label">{{ __('setting.Sex') }} <span
                class="text-danger">*</span></label>
        <div class="col-md-12">
            <select class="form-control" id="sex" name="sex" required="true">
                <option value="" style="display: none;"
                    {{ old('sex', optional($employee)->sex ?: '') == '' ? 'selected' : '' }}
                    disabled selected>{{ __('employee.Select Sex') }}</option>
...
<div class="row">
    ...
    <div class="form-group col-md-4 {{ $errors->has('status') ? 'has-error' : '' }}">
        <label for="status" class="col-md-12 control-label">{{ __('employee.Employee Status') }} <span
                class="text-danger">*</span></label>
        <div class="col-md-12">
            <select class="form-control" id="status" name="status" required="true">
                <option value="" style="display: none;"
                    {{ old('status', optional($employee)->status ?: '') == '' ? 'selected' : '' }}
                    disabled selected>
    ...
    <div class="form-group col-md-4 {{ $errors->has('status') ? 'has-error' : '' }}">
        <label for="status" class="col-md-12 control-label">Choose Role <span class="text-danger">*</span></label>
        <option value="" disabled="" {{ old('roles') ? '' : 'selected' }}></option>
        <select name="roles[]" class="select2 form-control select2-multiple " multiple="multiple"
            data-placeholder="Choose ...">

            @foreach ($roles as $role)
                <option value="{{ $role->id }}" {{ old('role') ? 'selected' : '' }}>{{ $role->name }}
                </option>
            @endforeach
    ...
    <div class="form-group col-lg-12 {{ $errors->has('password') ? 'has-error' : '' }}">
        <label for="password" class="col-md-12 control-label">{{ __('setting.password') }} <span
                class="text-danger">*</span></label>
        <div class="col-md-12">
            <input class="form-control" name="password" type="password" id="password"
                value="{{ old('password', optional($users)->password) }}" minlength="1" required="true"
                placeholder="{{ __('setting.password') }}">
        </div>
    </div>
</div>
</div>
