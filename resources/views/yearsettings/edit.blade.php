...
@extends('layouts.app')
@section('pagetitle')
    {{ __('setting.Edityear_setting') }}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#" ></a></li>
    <li class="breadcrumb-item"><a
            href="{{ route('competencylists.competencylist.index') }}">{{ __('setting.Addyear_setting') }}</a></li>
    <li class="breadcrumb-item active">{{ __('setting.edit') }}</li>
@endsection
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-1">{{ __('setting.Edityear_setting') }}</h3>
        </div>
        <div class="card-body">

            <form method="POST" action="{{ route('yearsettings.yearsetting.update', $yearsetting->id) }}" id="edit_yearsetting_form" name="edit_yearsetting_form" accept-charset="UTF-8" class="form-horizontal">
                {{ csrf_field() }}
